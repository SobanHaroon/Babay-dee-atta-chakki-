...
Set `SUPABASE_URL` and `SUPABASE_SECRET_KEY` (or `SUPABASE_SERVICE_ROLE_KEY`) in the hosting provider's server environment for both preview and production, then redeploy. The key must belong to the same project as the URL. Never put a secret/service-role key in a `VITE_` variable. The bundled public key is insufficient for protected order writes.

Checkout returns success only after the `orders` insert succeeds. The existing table must have `order id`, `customer name`, `contact number`, `customer address`, and `Price` columns; its primary `id` should be generated by the database. Optional legacy columns are removed from the insert only when Supabase reports them as absent. Full item, delivery, and fulfillment details are retained in the existing address metadata format.

Confirmation-page HTML receipts are generated from the confirmed order in the browser. The API receipt endpoint reads persisted orders after a cold start and returns 404 for unknown orders.

Run `npm run lint`, `npm run build`, and `npx tsx --test scripts/order-regression.test.ts` to validate the changes locally. The regression tests mock external services and never create real orders or send notifications.
