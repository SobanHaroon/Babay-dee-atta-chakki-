...
/**
 * Returns active SMSPK API Key (checks process.env.SMSPK_API_KEY).
 */
export function getSMSPKApiKey(): string {
  return (process.env.SMSPK_API_KEY || "").trim();
}
...
export function getSMSPKSenderId(): string {
  return (process.env.SMSPK_SENDER_ID || DEFAULT_SMSPK_SENDER_ID).trim();
}
...
export function isSMSPKConfigured(): boolean {
  return Boolean(getSMSPKApiKey()) && !smspkIpRestricted;
}
...
export async function sendSMSViaSMSPK(options: {
  ...
  if (!apiKey) {
    return {
      ...
      delivered: false,
      simulated: true,
      notice: "SMSPK API Key not set in environment. Running in simulation mode.",
      gateway: "smspk",
    };
  }
  ...
  if (!mobile || mobile.length < 10) {
    return {
      success: false,
      delivered: false,
      error: `Invalid recipient phone number format: ${options.mobile}`,
      gateway: "smspk",
    };
  }
  ...
  try {
    ...
    if (parsedJson) {
      if (parsedJson.success === "true" || parsedJson.success === true || parsedJson.response === "OK") {
        ...
        const isIpWhitelistNotice = typeof errorDetail === "string" && (
          errorDetail.toLowerCase().includes("whitelist your ip") ||
          errorDetail.toLowerCase().includes("api disabled")
        );

        if (isIpWhitelistNotice) {
          smspkIpRestricted = true;
          return {
            success: true,
            delivered: false,
            simulated: true,
            messageId: `smspk-sim-${Date.now()}`,
            notice: "SendPK gateway requires static IP whitelisting in profile dashboard. SMS simulated.",
            gateway: "smspk",
          };
        }
    ...
    return {
      success: false,
      delivered: false,
      error: responseText.slice(0, 150),
      responseRaw: responseText,
      gateway: "smspk",
    };
  } catch (err: any) {
...
export async function sendOrderConfirmationSMS(order: any): Promise<{
  ...
  if (isSMSPKConfigured() && smspkPhone) {
    try {
      ...
      if (smspkResult.success && smspkResult.delivered) {
      ...
      } else if (smspkResult.simulated) {
        console.log(`[SMS Gateway] Order confirmation SMS processed in simulation mode for Order #${order.id}`);
      }
    } catch {
      // Continue to Twilio or simulation fallback
    }
  }
  ...
  if (twilioClient && (fromNumber || messagingServiceSid) && normalizedPhone) {
    try {
  ...
  // 3. Simulated delivery fallback
  return {
    success: true,
    skipped: true,
    gateway: "simulation",
    recipientPhone: smspkPhone || normalizedPhone,
    message: "Order confirmation SMS recorded in simulation mode.",
  };
}
...
export async function sendOrderStatusSMS(
  ...
  if (!rawPhone) {
    return { success: true, notice: "No phone number available", gateway: "simulation" };
  }
