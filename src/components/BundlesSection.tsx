...
export function BundlesSection({ products, onAddBundleToCart }: BundlesSectionProps) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
        <div className="md:col-span-8 grid grid-cols-3 gap-2 sm:gap-3 items-center">
          {bundleItems.map((item, idx) => (
            <React.Fragment key={item.id}>
              <div className="bg-white dark:bg-slate-800/80 p-3 rounded-2xl border border-slate-200/80 dark:border-slate-700 shadow-2xs flex flex-col items-center text-center relative group">
                <ProductIcon
                  productId={item.id}
                  productImage={item.productImage}
                  size={20}
                  className="w-12 h-12 sm:w-16 sm:h-16 rounded-xl shadow-2xs mb-2 object-cover"
                />
                <h4 className="font-bold text-xs text-slate-800 dark:text-slate-200 line-clamp-1">{item.name}</h4>
                <span className="text-[11px] font-extrabold text-[#3b4414] dark:text-amber-400 mt-0.5">
                  Rs. {item.price}
                </span>
                {idx < 2 && (
                  <div className="absolute -right-3 sm:-right-4 top-1/2 -translate-y-1/2 z-10 w-6 h-6 rounded-full bg-amber-500 text-slate-950 flex items-center justify-center font-bold text-xs shadow-xs hidden sm:flex">
                    +
                  </div>
                )}
              </div>
        ...
        <div className="md:col-span-4 flex flex-col justify-center">
          <button
            ...
            className="w-full bg-[#3b4414] hover:bg-[#2c330e] text-white font-extrabold py-3.5 px-4 rounded-2xl shadow-lg flex items-center justify-center gap-2 transition-all active:scale-95 cursor-pointer text-sm group"
          >
            <Sparkles className="w-4 h-4 text-amber-400 group-hover:rotate-12 transition-transform" />
            <span>Add All 3 Items to Basket</span>
          </button>
        </div>
      </div>
    </section>
