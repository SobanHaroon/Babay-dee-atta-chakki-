...
import { searchGeoapifyPlaces, areAddressesMatching } from "../lib/mapUtils";
...
export function CheckoutMultiStepForm({
  ...
  const handleSelectAddressPrediction = (placeId: string, description: string, itemLat?: number, itemLng?: number, itemCity?: string, itemArea?: string) => {
    ...
    setCheckoutFormData((prev) => ({
      ...prev,
      address: description,
      confirmCompleteAddress: description,
      city: itemCity || prev.city || (description.toLowerCase().includes("islamabad") ? "Islamabad" : "Rawalpindi"),
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 1) {
      ...
      const emailVal = (checkoutFormData.email || "").trim();
      ...
      if (!customerCoordinates || isNaN(customerCoordinates.lat) || isNaN(customerCoordinates.lng) || (customerCoordinates.lat === 0 && customerCoordinates.lng === 0)) {
        errors.address = "Valid map location coordinates are required. Please select or pin your location on the map.";
      }
      ...
      if (!checkoutFormData.confirmCompleteAddress || !checkoutFormData.confirmCompleteAddress.trim()) {
        errors.confirmCompleteAddress = "Confirm Complete Address is mandatory. Please ensure both address fields match.";
      ...
      } else if (checkoutFormData.address && !areAddressesMatching(checkoutFormData.address, checkoutFormData.confirmCompleteAddress)) {
        errors.confirmCompleteAddress = "The 'Address' and 'Confirm Complete Address' fields do not match. Please verify both fields contain the exact same address.";
      }

      if (isSyncingMap) {
        errors.address = "Map synchronization in progress. Please wait for coordinates and delivery charges to update.";
      }
    }
            ...
            >
              ...
              <motion.div custom={direction} variants={fieldItemVariants} className="space-y-1">
                <div className="flex items-center justify-between">
                  <label
                    htmlFor="chk-form-email"
                    className="text-xs font-bold text-slate-600 uppercase tracking-wide block"
                  >
                    Email Address <span className="text-slate-400 font-normal normal-case">(for Order Receipt &amp; Invoice)</span>
                  </label>
                  <span className="text-[10px] text-[#3b4414] bg-[#3b4414]/10 px-2 py-0.5 rounded-full font-bold">
                    Email Receipt
                  </span>
                </div>
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setCheckoutFormData((prev) => ({ ...prev, email: e.target.value }));
                    }}
                ...
                {fieldErrors.email ? (
                  ...
                  <span className="text-[10px] text-slate-400 block">
                    We'll email a beautifully formatted order summary receipt &amp; live dispatch status immediately upon checkout.
                  </span>
                )}
              </motion.div>
          ...
          {currentStep === 2 && (
            ...
            >
                  ...
                  {checkoutFormData.email && (
                    <>
                      ...
                      <span className="text-slate-600 font-medium flex items-center gap-1">
                        <Mail className="w-3 h-3 text-[#3b4414]" />
                        <span className="truncate max-w-[140px]">{checkoutFormData.email}</span>
                      </span>
                    </>
                  )}
                </div>
                ...
                <>
                  ...
                  <motion.div custom={direction} variants={fieldItemVariants} className="space-y-2">
                    ...
                    <DeliveryMapPicker
                      ...
                      selectedLat={customerCoordinates?.lat}
                      selectedLng={customerCoordinates?.lng}
                      addressInput={checkoutFormData.address}
                      onLocationChange={(data) => {
                        ...
                        if (data.address) {
                          ...
                          setCheckoutFormData((prev: any) => ({
                            ...
                            confirmCompleteAddress: data.address,
              ...
              <motion.div custom={direction} variants={fieldItemVariants} className="space-y-1.5 relative">
                ...
                {/* Real-time Address Match Comparison Indicator */}
                {checkoutFormData.address && checkoutFormData.confirmCompleteAddress && (
                  <div className="pt-1">
                    {areAddressesMatching(checkoutFormData.address, checkoutFormData.confirmCompleteAddress) ? (
                      <div className="flex items-center gap-1.5 text-[11px] font-semibold text-emerald-800 bg-emerald-50 border border-emerald-200 px-3 py-1.5 rounded-lg">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                        <span>Addresses match and are synchronized with map pin.</span>
                      </div>
                    ) : (
                      <div className="flex items-center justify-between gap-2 text-[11px] font-semibold text-amber-900 bg-amber-50 border border-amber-300 p-2.5 rounded-lg">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
                          <span className="truncate">Both address fields must match before proceeding.</span>
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white rounded text-[10px] font-bold cursor-pointer transition-colors shadow-2xs whitespace-nowrap shrink-0"
                        >
                          Match with Map
                        </button>
                      </div>
              ...
              <motion.div custom={direction} variants={fieldItemVariants} className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2 text-xs text-slate-700">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-slate-850 text-right">
                    {checkoutFormData.name} ({checkoutFormData.phone})
                    {checkoutFormData.email && <span className="block font-normal text-[11px] text-slate-600">{checkoutFormData.email}</span>}
                  </span>
                </div>
