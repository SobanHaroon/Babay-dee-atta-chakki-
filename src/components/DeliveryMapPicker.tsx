...
export const DeliveryMapPicker: React.FC<DeliveryMapPickerProps> = ({
  ...
  const [customerCoords, setCustomerCoords] = useState<{ lat: number; lng: number }>(() => ({
    lat: selectedLat || initialLat || 33.567348,
    lng: selectedLng || initialLng || 73.104510
  }));

  const lastSyncedCoords = useRef<{ lat: number; lng: number }>({
    lat: selectedLat || initialLat || 33.567348,
    lng: selectedLng || initialLng || 73.104510
  });
  ...
  const deliveryCircleRef = useRef<L.Circle | null>(null);
  ...
  const calculateRoute = useCallback(
    async (lat: number, lng: number, addressHint?: string) => {
      ...
      // Dynamically resolve full address via reverse geocoding if not provided or placeholder
      let resolvedAddr = (addressHint || "").trim();
      let resolvedCity = "";
      let resolvedArea = "";

      if (
        !resolvedAddr ||
        resolvedAddr.startsWith("Pinned Location") ||
        resolvedAddr.startsWith("Location (") ||
        resolvedAddr.startsWith("Delivery Pin") ||
        resolvedAddr === "GPS Device Pin" ||
        resolvedAddr.length < 5
      ) {
        try {
          const revRes = await reverseGeocodeLatLng(safeLat, safeLng, GEOAPIFY_GEOCODING_KEY);
          if (revRes && revRes.address) {
            resolvedAddr = revRes.address;
            resolvedCity = revRes.city;
            resolvedArea = revRes.area;
          }
        } catch (revErr) {
          console.warn("Client reverse geocoding note:", revErr);
        }
      }

      if (!resolvedCity || !resolvedArea) {
        const parsed = extractCityAndArea(resolvedAddr, safeLat, safeLng);
      ...
      const applyLocalCalculation = () => {
        ...
        const fallbackArea = resolvedArea || (zoneInfo ? zoneInfo.zone.name : "Gulraiz Phase 3");
        const fallbackCity = resolvedCity || (zoneInfo ? zoneInfo.zone.city : (safeLat > 33.655 ? "Islamabad" : "Rawalpindi"));
      ...
      try {
        const res = await fetch("/api/delivery/calculate-route", {
          ...
          body: JSON.stringify({
            ...
            longitude: safeLng,
            address: resolvedAddr,
            forceReverseGeocode: !addressHint
          })
        });
        ...
        if (data && (data.success || data.distanceKm !== undefined)) {
          ...
          const zoneInfo = findClosestTwinCityZone(safeLat, safeLng);
          const finalCity = data.city || resolvedCity || (zoneInfo ? zoneInfo.zone.city : (safeLat > 33.655 ? "Islamabad" : "Rawalpindi"));
          const finalArea = data.area || resolvedArea || (zoneInfo ? zoneInfo.zone.name : "Gulraiz Phase 3");
          
          let finalAddr = data.customerLocation?.address && !data.customerLocation.address.startsWith("Location (") && !data.customerLocation.address.startsWith("Pinned Location")
            ? data.customerLocation.address
            : resolvedAddr;

          if (!finalAddr || finalAddr.startsWith("Pinned Location") || finalAddr.startsWith("Location (") || finalAddr.startsWith("Delivery Pin")) {
            finalAddr = `${finalArea}, ${finalCity}, Pakistan`;
          }
          ...
          setCustomerAddress(finalAddr);
          setDetectedCity(finalCity);
          setDetectedArea(finalArea);
    ...
    [storeInfo.lat, storeInfo.lng, storeInfo.pricePerKm, storeInfo.maxDeliveryDistanceKm, onLocationChange, customerCoords.lat, customerCoords.lng]
  );

  // Sync with selectedLat/selectedLng when updated by parent form (e.g. forward geocoding)
  useEffect(() => {
    if (
      ...
      const distDiff = Math.hypot(selectedLat - lastSyncedCoords.current.lat, selectedLng - lastSyncedCoords.current.lng);
      if (distDiff > 0.0001) {
        lastSyncedCoords.current = { lat: selectedLat, lng: selectedLng };
        setCustomerCoords({ lat: selectedLat, lng: selectedLng });
        if (customerMarkerRef.current) {
          customerMarkerRef.current.setLatLng([selectedLat, selectedLng]);
        }
        if (leafletMapRef.current) {
          leafletMapRef.current.flyTo([selectedLat, selectedLng], 15, { duration: 1.0 });
        }
        calculateRoute(selectedLat, selectedLng, addressInput);
      }
    }
  }, [selectedLat, selectedLng, addressInput, calculateRoute]);
  ...
  useEffect(() => {
    ...
    // Add Visual Store Delivery Radius Circle Layer (Immediate feedback on deliverability zone)
    const radiusMeters = (storeInfo.maxDeliveryDistanceKm || 30) * 1000;
    const deliveryCircle = L.circle([storeInfo.lat, storeInfo.lng], {
      radius: radiusMeters,
      color: "#3b4414",
      weight: 2,
      dashArray: "6, 6",
    ...
    deliveryCircle.bindTooltip(
      `<div style="font-family: sans-serif; font-size: 11px; font-weight: 700; color: #3b4414; padding: 2px 4px;">
        🌾 Store Delivery Zone (${storeInfo.maxDeliveryDistanceKm || 30} km)
       </div>`,
      { permanent: false, direction: "top", opacity: 0.95 }
    );
    deliveryCircleRef.current = deliveryCircle;
  ...
  useEffect(() => {
    ...
    // Update delivery radius circle position and radius
    if (deliveryCircleRef.current) {
      deliveryCircleRef.current.setLatLng([storeInfo.lat, storeInfo.lng]);
      deliveryCircleRef.current.setRadius((storeInfo.maxDeliveryDistanceKm || 30) * 1000);
    }
