...
export function LiveTrackingMap({ status, area, address, createdAt, orderId }: LiveTrackingMapProps) {
  ...
  // Calculate real-time progress based on the 5 canonical database statuses
  const realProgress = useMemo(() => {
    const s = (status || "").toLowerCase().trim();
    if (s.includes("delivered")) return 100;
    if (s.includes("out for delivery")) return 75;
    if (s.includes("dispatch")) return 50;
    if (s.includes("pending") || s.includes("milling") || s.includes("inspected")) return 25;
    return 5; // "order placed"
  }, [status]);
