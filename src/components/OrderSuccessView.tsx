...
function EmailReceiptCard({ order }: { order: Order }) {
  const initialEmail = order.customer?.email || (typeof window !== "undefined" ? localStorage.getItem("customer_email") || "" : "");
  const [emailInput, setEmailInput] = useState(initialEmail);
  const [isSending, setIsSending] = useState(false);
  
  // Track delivery status: set if order already had email confirmation delivered
  const [sendStatus, setSendStatus] = useState<{
    ...
    if (order.emailStatus?.delivered) {
      return {
        delivered: true,
        recipient: order.emailStatus.recipient || initialEmail,
        message: order.emailStatus.message || "Order receipt delivered to customer email address.",
      };
    }
    return null;
  });
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    setSendStatus(null);
    try {
      const res = await fetch("/api/order/send-receipt", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          orderId: order.id,
      ...
      if (data.success || data.delivered) {
        setSendStatus({
          ...
          message: data.message || `Receipt delivered to ${cleanEmail}`,
        });
        setShowInput(false);
        ...
        setSendStatus({
          delivered: false,
          error: data.error || data.message || "Failed to deliver email receipt."
        });
      }
    } catch (err: any) {
      setSendStatus({
        delivered: false,
        error: err.message || "Network error while dispatching receipt."
      });
    } finally {
      setIsSending(false);
    }
    ...
    <div className="bg-gradient-to-br from-blue-50/70 via-indigo-50/30 to-amber-50/40 border border-blue-200/80 rounded-2xl p-5 md:p-6 space-y-4 shadow-xs">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-sm font-black text-slate-900">Order Summary Email &amp; Tax Invoice</h4>
            <p className="text-xs text-slate-600 font-medium mt-0.5">
              Itemized receipt with stone-ground milling specs, pricing breakdown, and live tracking link.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
          <button
            type="button"
            onClick={handleOpenWebReceipt}
            className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-bold text-blue-700 hover:text-blue-800 bg-white hover:bg-blue-50/80 border border-blue-200 rounded-xl transition-all shadow-2xs cursor-pointer"
            title="View beautiful web version of the invoice"
          >
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-bold text-slate-700 hover:text-slate-900 bg-white hover:bg-slate-50 border border-slate-200 rounded-xl transition-all shadow-2xs cursor-pointer"
            title="Download offline invoice HTML file"
          >
      ...
      {/* Dispatched Delivery State */}
      {sendStatus?.delivered ? (
        <div className="bg-emerald-50/90 border border-emerald-200/80 rounded-xl p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
          <div className="flex items-start gap-2.5 text-emerald-950 font-medium">
            <div className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center shrink-0 mt-0.5">
              <Check className="w-3.5 h-3.5 stroke-[3]" />
            </div>
            <div>
              <span className="font-bold text-emerald-900">Receipt Dispatched:</span>{" "}
              <span>Sent to <span className="font-mono font-bold text-emerald-950">{sendStatus.recipient}</span></span>
              <span className="block text-[11px] text-emerald-700 mt-0.5">
                Check your inbox (or spam folder) for the complete formatted order breakdown.
              </span>
            </div>
          </div>
          <button
            ...
            className="text-emerald-800 hover:text-emerald-950 underline font-bold text-[11px] shrink-0 self-end sm:self-auto cursor-pointer"
          >
            {showInput ? "Hide form" : "Send copy to another email"}
          </button>
        </div>
      ) : null}

      {/* Clean Email Dispatch Form */}
      {showInput && (
        <form onSubmit={handleSend} className="space-y-2 pt-1 border-t border-blue-200/60">
          <label htmlFor="receipt-email-input" className="block text-[11px] font-bold text-slate-700 uppercase tracking-wide">
            {sendStatus?.delivered ? "Send copy to another email address:" : "Recipient Email Address for Receipt:"}
          </label>
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Enter customer email (e.g. karpeter09@gmail.com)"
                className="w-full text-xs pl-9 pr-3 py-2.5 bg-white border border-slate-300 rounded-xl outline-none text-slate-800 font-medium focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
                required
              />
            </div>
            <button
              type="submit"
              disabled={isSending}
              className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-bold text-xs rounded-xl inline-flex items-center justify-center gap-1.5 shadow-sm transition-all cursor-pointer shrink-0"
            >
              {isSending ? (
                <>
          ...
          {sendStatus?.error && (
            <p className="text-[11px] text-red-600 font-medium">{sendStatus.error}</p>
          )}
        </form>
      )}
    </div>
...
export function OrderSuccessView({ order, onClose, onTrack, onReorder }: OrderSuccessViewProps) {
  ...
  return (
    <div id="order-success-view-container" className="space-y-8 animate-fade-in max-w-2xl mx-auto">
          ...
          <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 space-y-4 text-xs">
            ...
            <div className="space-y-3">
              ...
              {order.customer.email && (
                <div className="flex items-start gap-2.5">
                  <Mail className="w-4 h-4 text-slate-450 shrink-0 mt-0.5 text-blue-600" />
                  <div>
                    <h4 className="font-bold text-slate-650">Email Address</h4>
                    <p className="text-slate-500 font-medium font-mono text-[11px]">{order.customer.email}</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div className="space-y-1.5 w-full">
                  <h4 className="font-bold text-slate-650">Delivery Destination (Map Verified)</h4>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="font-bold text-amber-900 bg-amber-100/80 border border-amber-200 px-2 py-0.5 rounded text-[11px]">
                      {order.customer.city || order.deliveryDetails?.city || (order.customer.area?.toLowerCase().includes("islamabad") ? "Islamabad" : "Rawalpindi")}
                    </span>
                    <span className="text-slate-400">•</span>
                    <span className="font-bold text-slate-800 bg-slate-150 px-2 py-0.5 rounded text-[11px]">
                      {order.customer.area || order.deliveryDetails?.area || "Gulraiz Phase 3"}
                    </span>
                  </div>
                  <p className="text-slate-700 leading-normal font-medium text-[11.5px] bg-white p-2 rounded-lg border border-slate-200 shadow-2xs">
                    {order.customer.address}
                  </p>
                  {(order.customer.latitude || order.deliveryDetails?.latitude) && (
                    <div className="flex items-center justify-between text-[10px] text-slate-500 font-mono bg-slate-100/70 p-1.5 rounded-md border border-slate-200/60">
                      <span>GPS Coordinates:</span>
                      <span className="font-bold text-slate-700">
                        {(order.customer.latitude || order.deliveryDetails?.latitude)?.toFixed(5)}, {(order.customer.longitude || order.deliveryDetails?.longitude)?.toFixed(5)}
                      </span>
                    </div>
                  )}
                </div>
      ...
      {/* Order Summary Email Receipt Card */}
      <EmailReceiptCard order={order} />
