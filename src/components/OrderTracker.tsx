      ...
      {order && (
        <div className="space-y-6 animate-fade-in mt-6">
          ...
          <div className="py-6 border-y border-slate-100">
            <h4 className="text-xs font-bold uppercase text-slate-400 tracking-wider mb-6 text-center">
              Milling & Dispatch Track Progress
            </h4>

            {/* Micro progress graph for desktop */}
            <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-2">
              ...
              <div className="absolute top-[18px] left-[15px] right-[15px] h-0.5 bg-slate-100 -z-10 hidden md:block" />
              
              {statusSteps.map((step) => {
                const currentIdx = getStatusIndex(order.status);
                const isCompleted = getStatusIndex(step.label) <= currentIdx;
                const isActive = step.label === order.status;
                const StepIcon = step.icon;

                return (
                  <div key={step.label} className="flex md:flex-col items-center gap-3 md:gap-2.5 md:text-center flex-1">
                    <div
                    ...
                    <div className="text-left md:text-center mt-1">
                      ...
                      <p className="text-[10px] text-slate-400 max-w-[124px] mt-0.5 hidden md:block">
                        {step.desc}
                      </p>
                    </div>
                  </div>
