...
import { Search, Loader2, Clock, CheckCircle2, MapPin, Truck, Box, Check } from "lucide-react";
...
export function OrderTracker() {
  ...
  const [orderIdInput, setOrderIdInput] = useState(() => {
    if (typeof window !== "undefined") {
      ...
      const urlOrder = urlParams.get("order") || urlParams.get("orderId") || urlParams.get("track");
      if (urlOrder) return urlOrder.toUpperCase();
      if (window.location.hash.includes("track=")) {
        return window.location.hash.split("track=")[1].split("&")[0].toUpperCase();
      }
  ...
  // Sync if URL search params or hash change while mounted
  useEffect(() => {
    if (typeof window !== "undefined") {
      const urlParams = new URLSearchParams(window.location.search);
      const urlOrder = (urlParams.get("order") || urlParams.get("orderId") || urlParams.get("track") || "").trim().toUpperCase();
      let targetId = urlOrder;
      if (!targetId && window.location.hash.includes("track=")) {
        targetId = window.location.hash.split("track=")[1].split("&")[0].trim().toUpperCase();
      }
      if (targetId && targetId !== trackingId) {
        setOrderIdInput(targetId);
        setTrackingId(targetId);
        localStorage.setItem("last_tracking_id", targetId);
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    // Listen for realtime orders changes broadcast from App.tsx
    const handleRealtimeUpdate = (e: Event) => {
      const customEvent = e as CustomEvent;
      const payload = customEvent.detail;
      const targetNumeric = String(trackingId).replace("BDEC-", "").trim();
      if (payload && payload.new && String(payload.new["order id"]) === targetNumeric) {
        fetchOrder(false);
      } else if (!payload) {
        fetchOrder(false);
      }
    ...
    // Subscribe directly to Supabase Realtime channel for this specific order in the 'orders' table
    let channel: any = null;
    if (supabase) {
      const numericId = parseInt(String(trackingId).replace("BDEC-", "").trim(), 10);
      if (!isNaN(numericId)) {
        channel = supabase
          .channel(`order-track-realtime-${numericId}`)
          .on(
            "postgres_changes",
            {
              event: "*",
    ...
    // Polling interval backup every 10 seconds to ensure strictly synchronised status
  ...
  // 5 Canonical lifecycle stages requested by the user:
  // ( order placed, pending, dispatch, out for delivery and delivered )
  const statusSteps = [
    { key: "order placed", label: "Order Placed", desc: "Received and confirmed", icon: Box },
    { key: "pending", label: "Pending", desc: "Queued & pending preparation", icon: Clock },
    { key: "dispatch", label: "Dispatch", desc: "Milled & dispatched from depot", icon: CheckCircle2 },
    { key: "out for delivery", label: "Out for Delivery", desc: "Rider is heading to your address", icon: Truck },
    { key: "delivered", label: "Delivered", desc: "Package handed over safely", icon: MapPin }
  ];
  ...
  const getStatusIndex = (currentStatus: string): number => {
    ...
    if (s.includes("out for delivery") || s.includes("on the way")) return 3;
    if (s.includes("dispatch")) return 2;
    if (s.includes("pending") || s.includes("milling") || s.includes("processing") || s.includes("inspected")) return 1;
    if (s.includes("order placed") || s.includes("placed") || s.includes("received")) return 0;
    return 0;
  };
        ...
        <div className="space-y-6 animate-fade-in mt-6">
          ...
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            ...
            <div className="text-left md:text-right">
              ...
              <span className="inline-flex items-center gap-1.5 text-xs font-black text-emerald-700 bg-emerald-50 px-3 py-1 rounded-full border border-emerald-100 mt-0.5 capitalize">
          ...
          {/* 5-Step Order Lifecycle Stepper (order placed, pending, dispatch, out for delivery, delivered) */}
          <div className="bg-slate-50/70 border border-slate-100 rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                <h5 className="text-xs font-bold text-slate-700 tracking-tight uppercase">Order Status</h5>
              </div>
            </div>
            ...
            <div className="relative pt-2 pb-1">
              {/* Progress Line */}
              <div className="absolute top-7 left-6 right-6 h-1 bg-slate-200 -translate-y-1/2 z-0 hidden sm:block">
                <div
                  className="h-full bg-emerald-600 transition-all duration-500"
                  style={{
                    width: `${(Math.min(currentStepIdx, 4) / 4) * 100}%`
                  }}
                />
              </div>
                ...
                {statusSteps.map((step, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-slate-500 leading-tight mt-0.5 line-clamp-2 hidden sm:block">
                        {step.desc}
                      </span>
                      {isCurrent && (
                        <span className="mt-1.5 text-[9px] font-bold uppercase tracking-wider text-emerald-700 bg-emerald-100/70 px-2 py-0.5 rounded-full">
                          Current Status
                        </span>
                      )}
                    </div>
