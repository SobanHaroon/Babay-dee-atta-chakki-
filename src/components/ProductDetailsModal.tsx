...
import { Star, X, ShoppingCart, Sparkles, Share2, ChevronLeft, ChevronRight } from "lucide-react";
...
export function ProductDetailsModal({
  ...
  // Calculate current product index within catalog for Prev / Next browsing
  ...
  const totalNum = allProducts.length;
  ...
  // Dynamic Product Image URL for the atmospheric background
  const bgImageUrl = product.productImage || product.img || "/logo%20coloured.jpg";
  ...
  return (
    <div
      ...
      className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-2.5 sm:p-4 overflow-y-auto"
      onClick={(e) => {
    ...
    >
      ...
      >
        {/* =========================================================
            1. DYNAMIC BLURRED PRODUCT IMAGE BACKGROUND + DARK OVERLAY
            ========================================================= */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none z-0 rounded-3xl">
          ...
          {/* Dark & Palette-Tuned Semi-Transparent Gradient Overlay for Readability */}
          <div 
            className="absolute inset-0 bg-gradient-to-b from-stone-950/75 via-stone-950/60 to-stone-950/85 pointer-events-none"
            style={{
        ...
        {/* =========================================================
            NAVIGATION CONTROLS: Previous & Next Product Arrows
            ========================================================= */}
        ...
        <button
          ...
          disabled={!hasPrev}
          id="product-overview-prev-btn"
          aria-label="Previous product"
          title={hasPrev ? `Previous: ${allProducts[currentIndex - 1]?.name}` : "First product"}
          className={cn(
            "absolute left-2.5 sm:left-3.5 top-1/2 -translate-y-1/2 z-40 w-11 h-11 sm:w-12 sm:h-12 min-w-[44px] min-h-[44px] rounded-full flex items-center justify-center transition-all duration-200 shadow-2xl",
            "bg-white/85 hover:bg-white text-slate-800 hover:text-amber-700 backdrop-blur-md border border-white/60",
            "active:scale-95 cursor-pointer",
            !hasPrev && "opacity-20 cursor-not-allowed pointer-events-none shadow-none"
          )}
        >
        ...
        <button
          ...
          disabled={!hasNext}
          id="product-overview-next-btn"
          aria-label="Next product"
          title={hasNext ? `Next: ${allProducts[currentIndex + 1]?.name}` : "Last product"}
          className={cn(
            "absolute right-2.5 sm:right-3.5 top-1/2 -translate-y-1/2 z-40 w-11 h-11 sm:w-12 sm:h-12 min-w-[44px] min-h-[44px] rounded-full flex items-center justify-center transition-all duration-200 shadow-2xl",
            "bg-white/85 hover:bg-white text-slate-800 hover:text-amber-700 backdrop-blur-md border border-white/60",
            "active:scale-95 cursor-pointer",
            !hasNext && "opacity-20 cursor-not-allowed pointer-events-none shadow-none"
          )}
        >
        ...
        <button
          ...
          className="absolute top-3.5 right-3.5 sm:top-4 sm:right-4 bg-white/90 hover:bg-white text-slate-700 hover:text-slate-950 w-10 h-10 min-w-[40px] min-h-[40px] flex items-center justify-center rounded-full cursor-pointer transition-all z-40 shadow-md active:scale-95 border border-white/60 backdrop-blur-xs"
        >
          <X className="w-5 h-5" />
        </button>

        {/* =========================================================
            LEFT HALF: 50% Full Area Product Visual Stage
            ========================================================= */}
        <div className="overview-left-half md:w-1/2 p-4 sm:p-6 lg:p-7 flex flex-col items-center justify-between border-b md:border-b-0 md:border-r border-slate-200/40 w-full shrink-0 overflow-y-auto max-h-[48vh] md:max-h-[92vh] relative z-10">
          <ProductViewer360 product={product} />
        </div>

        {/* =========================================================
            RIGHT HALF: 50% Detailed Content & Actions Scrollable (High Contrast)
            ========================================================= */}
        <div
          ref={scrollContentRef}
          className="overview-right-half md:w-1/2 bg-white/95 backdrop-blur-xl p-5 sm:p-7 md:p-8 overflow-y-auto max-h-[52vh] md:max-h-[92vh] space-y-6 flex flex-col justify-between relative z-10 shadow-lg border-t md:border-t-0 md:border-l border-slate-200/80"
        >
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-mono text-slate-400 font-bold ml-auto bg-slate-100/80 px-2 py-0.5 rounded-md border border-slate-200/60">
                  Product {currentNum} of {totalNum}
                </span>
              </div>
