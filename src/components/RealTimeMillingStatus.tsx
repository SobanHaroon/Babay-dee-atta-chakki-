...
export function RealTimeMillingStatus({
  ...
  // Map textual order status from database to step index (0 to 4)
  const mapStatusToStepIndex = (statusStr: string): number => {
    const s = (statusStr || "").toLowerCase().trim();
    if (s.includes("delivered") || s.includes("completed")) return 4;
    if (s.includes("out for delivery") || s.includes("on the way")) return 4;
    ...
    if (s.includes("placed") || s.includes("received") || s.includes("confirmed")) return 0;
