...
export function OrbitalImageWheel({
  ...
  }, [isPlaying, total, autoPlaySpeed, activeIndex]);

  // Trigger parent onSelect
  useEffect(() => {
    if (images[activeIndex] && onSelect) {
      onSelect(images[activeIndex], activeIndex);
    }
  }, [activeIndex, images, onSelect]);
        ...
        <div className="relative z-30 w-52 sm:w-64 md:w-72 p-3.5 sm:p-5 lg:p-6 bg-white/95 backdrop-blur-md rounded-2xl sm:rounded-3xl border border-amber-200/80 shadow-2xl flex flex-col items-center text-center space-y-2 sm:space-y-3.5 transition-all duration-300 hover:shadow-amber-500/10">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="relative w-20 h-20 sm:w-28 sm:h-28 lg:w-32 lg:h-32 rounded-xl sm:rounded-2xl bg-gradient-to-br from-amber-50 via-slate-50 to-stone-100 border border-slate-200/80 p-1.5 sm:p-2 shadow-inner flex items-center justify-center overflow-hidden">
                {activeItem.src ? (
                  <img
                    src={activeItem.src}
              ...
              <div className="space-y-0.5 sm:space-y-1 w-full">
                <h4 className="text-sm sm:text-base lg:text-lg font-black text-slate-950 tracking-tight leading-snug">
                  {activeItem.label}
                </h4>
                {activeItem.subtitle && (
              ...
              {activeItem.price && (
                <div className="text-xs sm:text-sm font-black text-[#3b4414] bg-amber-100/80 px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full border border-amber-300/60">
              ...
              <div className="w-full flex flex-col gap-1.5 sm:gap-2 pt-0.5">
                {onAddToCart && (
                  <button
                    ...
                    className="w-full py-2 sm:py-2.5 bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 active:scale-95 text-slate-950 font-black text-[10px] sm:text-[11px] uppercase tracking-wider rounded-lg sm:rounded-xl shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-1 sm:gap-1.5 cursor-pointer"
                  >
